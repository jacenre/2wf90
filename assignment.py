...
def add(x, y, radix):
    ...
    if negative:
        answer = "-" + answer                   # if negative is true, add minus sign to answer
...
def subtract(x, y, radix):
    ...
    if carry < 0:
        ...
        for i in range(0,len(answer)):
            answer[i] = numberToHex(str((radix - 1) - int(hexToNumber(answer[i]))))
        answer = ''.join(answer)
...
def multiply(x, y, radix):
    countAdd = 0                            # count total number of elementary additions/subtractions
    countMult = 0                           # count total number of elementary multiplications
...
def modular_reduction(x, m, radix):
    ...
    for i in range(k - n, -1, -1):
        ...
        while x_subtract_left_shifted_m[0] != "-":
            x_prime = x_subtract_left_shifted_m                                     # x' = x - m * b^i
            x_subtract_left_shifted_m = subtract(x_prime, m_left_shift, radix)      # update for new x'

    if (negative == False) or (x_prime == '0'):             # if x > 0 or x' = 0
        result = x_prime                                    # return x'
    else:
        result = subtract(m, x_prime, radix)                # else return m - x' 
    return result
...
def modular_inversion(x, m, radix):
    ...
    while m_prime != "0" and m_prime[0] != "-":                 # while m' > 0
        q = division_without_rest(a_prime, m_prime, radix)      # q = math.floor(a_prime / m_prime)

        q_mult_m_prime = multiply(q, m_prime, radix)            # q * m'
        q_mult_m_prime = q_mult_m_prime[0]                      # multiply() returns an array, take only first element

        r = subtract(a_prime, q_mult_m_prime, radix)            # r = a' - q * m'

        a_prime = m_prime                                       # a' = m'
        m_prime = r                                             # m' = r

        q_mult_x_2 = multiply(q, x_2, radix)                    # q * x_2
        q_mult_x_2 = q_mult_x_2[0]                              # multiply() returns an array, take only first element

        x_3 = subtract(x_1, q_mult_x_2, radix)                  # x_3 = x_1 - q * x_2
        x_1 = x_2                                               # x_1 = x_2
        x_2 = x_3                                               # x_2 = x_3
    if a_prime == "1":
        ...
        if x_1[0] == "-":                                       
            x_1 = subtract(m, x_1[1:], radix)                   # m - x_1 returns positive inverse modulo
        return x_1
    else:
        return "inverse does not exist"                         # if no inverse exists, error
...
def karatsuba(x, y, radix, countAdd, countMult):
    ...
    return result + answer[0], answer[1], answer[2] #(result, countAdd, countMult)
...
def compute_karatsuba(x, y, radix, countAdd, countMult):
    ...
    ac = compute_karatsuba(a, c, radix, countAdd, countMult)  # recurse to find a*c
    countAdd = ac[1]
    countMult = ac[2]
    ac = ac[0]

    bd = compute_karatsuba(b, d, radix, countAdd, countMult)  # recurse to find b*d
    countAdd = bd[1]
    countMult = bd[2]
    bd = bd[0]
    ...
    e = compute_karatsuba(xsum, ysum, radix, countAdd, countMult)  # recurse to find (a+b)*(c+d)
    countAdd = e[1]
    countMult = e[2]
    e = e[0]
...
def return_answer_euclid(d, a, b):
    output_file.write('[answ-d] ' + str(d) + '\n')
    output_file.write('[answ-a] ' + str(a) + '\n')
    output_file.write('[answ-b] ' + str(b) + '\n')
    output_file.write('\n')
        ...
        while line:
            if line[0] == '#' or line[0] == ' ':
                ...
                if len(words) > 0:
                    ...
                    if words[0] == '[euclid]':
                        el_x, el_y, el_m = read_x_y_m()
                        d, a, b =  euclid(el_x, el_y, el_radix)
                        return_answer_euclid(d, a, b)
                    if words[0] == '[karatsuba]':
                        el_x, el_y, el_m = read_x_y_m()
                        ans, count1, count2 = karatsuba(el_x, el_y, el_radix, 0, 0)
                        return_answer_counters(ans, count1, count2)
